...
public class GameOfThreeSocketEndpoint {
	...
	private void playingAGame(Session session, Message message) throws IOException, EncodeException {
        ...
        String nextTurnPlayer = playground.getCurrentPlayer().equals(playground.getPlayer1()) 
    			? playground.getPlayer2() : playground.getPlayer1();
...
//        sendMessage(message, gameUserEndpointMap.get(users.get(session.getId())));
    ...
    private static void sendMessage(Message message, GameOfThreeSocketEndpoint endpoint) throws IOException, EncodeException {
